...
openssl req -new -x509 -days 365 -key ca.key \
  -subj "/C=AU/CN=kubernetes-metadata-injector"\
  -out ca.crt
...
openssl x509 -req \
  -extfile <(printf "subjectAltName=DNS:kubernetes-metadata-injector.default.svc") \
  -days 365 \
  -in server.csr \
...
kubectl create secret tls kubernetes-metadata-injector-tls \
  --cert=server.crt \
  --key=server.key \
